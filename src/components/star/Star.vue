<template>
  <div class="star">
    <span
      class="star-item"
      v-for="(itemClass,index) in itemClasses"
      :key="index"
      :class="itemClass"
    ></span>
  </div>
</template>

<script>
//星星长度
// const LENGTH = 5;
//星星的状态  类名 对应的类名就可以往数组里push
const CLS_ON = 'on'
const CLS_HALF = 'half'
const CLS_off = 'off'

export default {
  props: {
    score: {
      type: Number
    }
  },
  //如果得到全星的，就把全星的类放到数组里面去
  //如果得到半星的，就把半星的类放到数组里面去
  //然后遍历数组的时候就能告诉你有几个全星几个半星了

  //处理五角星评分为什么要写在computed计算属性里去？？？
  computed: {
    itemClasses() {
      let result = []

      //   4.7
      let score = Math.floor(this.score * 2) / 2 //4.5

      //半星  如果得到半星的，就把半星的类放到数组里面去
      let hasDecimal = score % 1 !== 0

      //全星  如果得到全星的，就把全星的类放到数组里面去
      let integer = Math.floor(score) //4

      //遍历全星
      for (let i = 0; i < integer; i++) {
        result.push(CLS_ON)
      }

      //处理半星
      if (hasDecimal) {
        result.push(CLS_HALF)
      }

      //补齐
      while (result.length < 5) {
        result.push(CLS_off)
      }

      return result
    }
  },
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
.star {
  font-size: 0;
}
.star .star-item {
  display: inline-block;
  width: 10px;
  height: 10px;
  margin-right: 3px;
  background-repeat: no-repeat;
  background-size: 10px 10px;
}
.star .star-item:last-child {
  margin-right: 0;
}

/* 三种图片类型*/
.star .on {
  background-image: url(img/star24_on@2x.png);
}
.star .half {
  background-image: url(img/star24_half@2x.png);
}
.star .off {
  background-image: url(img/star24_off@2x.png);
}
</style>
